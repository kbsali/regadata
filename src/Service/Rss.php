...
class Rss
{
    ...
    public function generate()
    {
           ...
           $channel
               ...
               ->url($this->urlGenerator->generate('homepage', array(), true))
               ->language('en')
           ...
           foreach ($reports as $reportId) {
               ...
               $item
                   ->title($this->translator->trans('General ranking %date%', array('%date%' => date('Y-m-d H:i', $ts)), 'messages', 'en'))
                   // ->description("<div>Blog body</div>")

                   // ->url($app->url('report', array('id' => $report)))
                   ->url($this->urlGenerator->generate('report', array('id' => $reportId), true))

                   // ->guid($app->url('report', array('id' => $report)), true)
                   ->guid($this->urlGenerator->generate('report', array('id' => $reportId), true), true)
